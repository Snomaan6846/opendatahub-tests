...
def validate_image_output(response: requests.Response) -> bytes:
    """Validate a /v1/images/generations response and return decoded image bytes.

    Args:
        response: HTTP response from the image generation endpoint.
    ...
    assert response.status_code == 200, f"Expected HTTP 200, got {response.status_code}: {response.text[:200]}"
    data = response.json()
    assert "data" in data and len(data["data"]) >= 1, "Response missing 'data' array"
    b64 = data["data"][0].get("b64_json", "")
    assert b64, "b64_json field is empty"
    image_bytes = base64.b64decode(s=b64)
    assert image_bytes[:4] == _PNG_MAGIC or image_bytes[:3] == _JPEG_MAGIC, (
        f"Decoded bytes do not have PNG or JPEG magic: {image_bytes[:4]!r}"
    )
    return image_bytes
