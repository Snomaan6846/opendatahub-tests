...
@pytest.fixture(scope="session")
def skip_if_no_vllm_omni_multi_gpu(supported_accelerator_type: str | None) -> None:
    ...
    if not supported_accelerator_type or supported_accelerator_type.lower() != AcceleratorType.NVIDIA:
        pytest.skip(f"vLLM-Omni requires 2× NVIDIA GPUs. Found accelerator: '{supported_accelerator_type or 'None'}'.")
...
def vllm_omni_serving_runtime(
    ...
    template_name = OMNI_TEMPLATE_MAP.get(accelerator_type, OMNI_TEMPLATE_MAP[AcceleratorType.NVIDIA])
