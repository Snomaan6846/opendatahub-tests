...
_EXTERNAL_EGRESS_HOST: str = "huggingface.co"
...
class TestVllmOmniTtsQwen3:
    ...
    def test_vllm_omni_tts_qwen3_20_word_no_timeout(
        ...
        response = run_tts_inference(
            isvc=vllm_omni_inference_service,
            text=TWENTY_WORD_PROMPT,
            timeout=30,
        )
        assert response.status_code != 504, (
            "POST /v1/audio/speech with a 20-word prompt returned HTTP 504 Gateway Timeout; "
            "request must complete within the 30-second OpenShift Route default timeout"
        )
    ...
    def test_vllm_omni_no_pii_in_logs(
        ...
        assert predictor_pods, "No predictor pods found for vLLM-Omni ISVC"
    ...
    def test_vllm_omni_egress_network_policy_blocks_external(
        ...
        When an egress NetworkPolicy is applied restricting outbound traffic to ports 443 and 6443
        Then TTS inference via /v1/audio/speech returns HTTP 200 (weights are in pod memory),
             and an outbound TCP connection from the predictor pod to huggingface.co fails.
        """
        isvc_url = vllm_omni_isvc_url
        model_name = vllm_omni_inference_service.name
        ...
        with NetworkPolicy(
            ...
            egress_probe = exec_pod.execute(
                command=[
                    ...
                    (
                        ...
                        f"s.connect(('{_EXTERNAL_EGRESS_HOST}', 80)); "
                        "sys.exit(0)"
                    ),
                ],
                container=Containers.KSERVE_CONTAINER_NAME,
            ...
            assert egress_probe.rc != 0, (
                f"Outbound TCP connection to {_EXTERNAL_EGRESS_HOST}:80 was NOT blocked by "
    ...
    def test_vllm_omni_monitoring_user_can_read_metrics(
        self,
        admin_client: DynamicClient,
        model_namespace: Namespace,
        vllm_omni_inference_service: InferenceService,
        ...
        """Verify the /metrics endpoint exposes Prometheus data from the vLLM-Omni service.

        Given a vLLM-Omni InferenceService with metrics enabled (default)
        When the /metrics endpoint is queried via HTTP GET
        Then the response is HTTP 200 and the body contains vllm metric family names.

        Note: This test uses unauthenticated HTTP because role-based access control
        is enforced at the pod level by the kube-rbac-proxy sidecar, not by the
        OpenShift Route.  The Route exposes the metrics endpoint without additional
        auth, so an unauthenticated GET is the correct way to verify that the
        endpoint is reachable and returns vLLM metric families.
        """
        isvc_url = vllm_omni_isvc_url
