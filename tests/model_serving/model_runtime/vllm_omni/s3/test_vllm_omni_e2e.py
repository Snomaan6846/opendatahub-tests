...
class TestVllmOmniFullE2E:
    ...
    def test_vllm_omni_serving_runtime_dashboard_annotation(
        self,
        ...
        When its metadata annotations are inspected,
        Then the opendatahub.io/dashboard annotation must equal "true" so that the
    ...
    def test_vllm_omni_chat_inference(
        ...
        assert len(content) > 0, "Chat response content is empty; expected non-empty text"
    ...
    def test_vllm_omni_no_external_network_access_in_pod_logs(
        ...
        Then no DNS failures, download errors, or connection timeouts must be present,
        confirming the runtime operated without external network access.
        """
        logs: str = vllm_omni_pod_logs
        ...
        assert not detected_patterns, (
            ...
            f"external access attempts: {detected_patterns}. "
            "The runtime must not contact external networks."
        )
    ...
    def test_vllm_omni_models_endpoint(
        ...
        assert len(data) >= 1, "Expected at least one model entry in 'data' array"
    ...
    def test_vllm_omni_chat_completions_endpoint(
        ...
        assert len(choices) >= 1, "Expected at least one entry in 'choices' array"
    ...
    def test_vllm_omni_metrics_endpoint_exposed(
        ...
        Then vllm_omni:* metric families (>= 7) are present at /metrics, and Prometheus
        ...
        assert raw_metrics, f"Empty /metrics response from pod {vllm_omni_pod_resource.name}"
        ...
        prometheus_result = prometheus.query_sampler(query=PROMETHEUS_VALIDATION_METRIC)
        assert prometheus_result, (
            f"Prometheus returned no data for '{PROMETHEUS_VALIDATION_METRIC}'; "
