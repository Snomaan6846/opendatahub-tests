...
class TestVllmOmniTemplateStructure:
    ...
    def test_vllm_omni_runtime_template_exists(
        ...
        """Confirm the vLLM-Omni template is deployed and uses a sha256 digest image.
        ...
        And spec.containers[0].image uses a @sha256: digest reference injected via RELATED_IMAGE
        ...
        assert containers, "No containers found in vLLM-Omni template spec"
        ...
        LOGGER.info(event="vLLM-Omni template exists with sha256 digest image", image=container_image)
    ...
    def test_vllm_omni_dashboard_discovery_annotations(
        ...
        assert supported_formats, "spec.supportedModelFormats is empty in the vLLM-Omni template"
        vllm_format = next(
