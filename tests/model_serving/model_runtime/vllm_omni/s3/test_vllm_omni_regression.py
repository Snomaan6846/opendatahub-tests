...
def _get_supported_model_formats(template: Template) -> list[dict[str, Any]]:
    """Extract supportedModelFormats from a ServingRuntime template.

    Args:
        template: A Template resource whose first embedded object is a ServingRuntime.

    Returns:
        list[dict[str, Any]]: Supported model format entries, empty list if absent.
    """
    model_dict: dict[str, Any] = template.instance.objects[0].to_dict()
    return list(model_dict.get("spec", {}).get("supportedModelFormats", []))
...
class TestVllmOmniDoesNotBreakExistingCudaTemplate:
    ...
    def test_vllm_omni_does_not_break_existing_vllm(
        ...
        assert OMNI_FLAG not in cuda_args, (
            f"vLLM CUDA template '{RuntimeTemplates.VLLM_CUDA}' must not contain '{OMNI_FLAG}' "
            f"in its container args, but found it in: {cuda_args}. "
