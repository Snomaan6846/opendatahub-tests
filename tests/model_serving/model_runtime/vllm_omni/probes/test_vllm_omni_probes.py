...
from utilities.constants import Containers, KServeDeploymentType, Labels, Ports
...
class TestVllmOmniFailedStageInit:
    ...
    def test_vllm_omni_failed_stage_init_stays_not_ready(
        ...
        pod_logs = ""
        try:
            pod_logs = pods[0].log(container=Containers.KSERVE_CONTAINER_NAME)
        except Exception:  # noqa: BLE001
            LOGGER.debug(event="could not retrieve logs from crashed pod")
        ...
        LOGGER.info(
            ...
            log_tail=pod_logs[-500:] if pod_logs else "(no logs)",
        )
