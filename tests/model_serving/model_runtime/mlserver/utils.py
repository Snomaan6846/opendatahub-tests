...
from tests.model_serving.model_runtime.mlserver.constant import MLSERVER_GRPC_REMOTE_PORT, LOCAL_HOST_URL, PROTO_FILE_PATH
...
def run_mlserver_inference(
    ...
    if deployment_mode == KServeDeploymentType.RAW_DEPLOYMENT:
        ...
        with portforward.forward(pod_or_service=pod_name, namespace=isvc.namespace, from_port=port, to_port=port):
            host = f"{LOCAL_HOST_URL}:{port}" if is_rest else f"{LOCAL_HOST_URL.replace('http://', '')}:{port}"
            return send_rest_request(f"{host}{rest_endpoint}", input_data) if is_rest else send_grpc_request(host, input_data, root_dir)
        ...
        if is_rest:
            ...
            grpc_url = f"{base_url.replace('https://', '').replace('http://', '')}:{MLSERVER_GRPC_REMOTE_PORT}"
